...
function twenty8teen_widgets_init() {
	...
	register_sidebar( array(
		...
		'description'   => esc_html__( 'Displayed at the bottom. If empty, the site info is shown.', 'twenty8teen' ),
		'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget'  => '</div>',
		'before_title'  => '<h3 class="widget-title">',
		'after_title'   => '</h3>',
...
function twenty8teen_enqueue() {
	...
	if ( 'wp_enqueue_scripts' === current_action() ) {
		...
		wp_enqueue_script( 'twenty8teen-mouse-xy', get_template_directory_uri() .
			'/js/mouse-xy.js', array( 'prefixfree-vars' ), '20200614', true );
		...
		wp_enqueue_style( 'twenty8teen-editor', get_template_directory_uri() .
			'/css/editor-style.css', false, '20200329', 'all' );
...
( function() {
	var insertListener = function(event) {
		if (event.animationName == "nodeInserted") {
			var el = document.querySelector('.editor-writing-flow, .block-editor-writing-flow');
...
JS;
		$classes = twenty8teen_area_classes( 'main', 'site-main', false ) . ' ' .
			twenty8teen_area_classes( 'content', 'content-area', false );
		$classes = str_replace( ' ', '", "', $classes );
		$script = sprintf( $init_script, $classes );
...
add_action( 'enqueue_block_editor_assets', 'twenty8teen_enqueue' );
